...
(function () {
  ...
  const settings = (() => {
    if (DEFAULT.keepParameter) {
      const env = $.getenv('doujinhelper/autoFolio')
  ...
  const defaultColor = activeDocument.mode === DocumentMode.RGB ? settings.colorRGB : settings.colorCMYK
  ...
  const inputVertical = new FolioCoords(dialog, 'vertical', settings.verticalMode, settings.verticalSpace)
  const inputHorizontal = new FolioCoords(dialog, 'horizontal', settings.horizontalMode, settings.horizontalSpace)
  ...
  if (DEFAULT.keepParameter) {
    $.setenv('doujinhelper/autoFolio', JSON.stringify({
      ...
      bleed,
      verticalMode,
      verticalSpace: inputVertical.getValue() ?? settings.verticalSpace ?? DEFAULT.verticalSpace,
      horizontalMode,
      horizontalSpace: inputHorizontal.getValue() ?? settings.horizontalSpace ?? DEFAULT.horizontalSpace,
      binarization,
      flatten,
